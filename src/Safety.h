...
class Safety
{
	public:
		...
		float getManipulatorCurrent(unsigned ch) { return (ch < NUM_MANIPULATOR_MOTORS) ? lastManipulatorControlCurrent[ch] : 0; }
	...
	private:
		...
		float lastManipulatorSafetyCurrent[NUM_MANIPULATOR_MOTORS];
		float lastDriveControlCurrent[NUM_DRIVE_MOTORS];
		float lastManipulatorControlCurrent[NUM_MANIPULATOR_MOTORS];
